...
void TitleScene::OnEnter()
{
	...
	Widget::Register(&mAchievement);
	Widget::Register(&mExit);

	mTest.font = mExit.font =mAchievement.font = mFont;
	mTest.text = "Test";
	mExit.text = "Exit";
	mAchievement.text = "Achievement";
	mTest.textColor = { 0, 255, 0, 255 };
	mExit.textColor = { 255, 0, 0, 255 };
	mAchievement.textColor = { 255, 255, 0, 255 };
	const float buttonWidth = 60.0f;
	const float buttonHeight = 40.0f;
	const float achievementButtonWidth = 150.0f; // Increase the width
	const float achievementButtonHeight = 40.0f; // Increase the height
	...
	mAchievement.rect.w = achievementButtonWidth;
	mAchievement.rect.h = achievementButtonHeight;
	mAchievement.rect.x = buttonMargin; // Position a bit from the left edge
	mAchievement.rect.y = buttonMargin; 
	// Set "Test" button interactions
	...
	mAchievement.SetOnIn([](void* data) {
		LabelledButton& btn = *(LabelledButton*)data;
		btn.text = "Achievement";
		}, &mAchievement);
	mAchievement.SetOnOut([](void* data) {
		LabelledButton& btn = *(LabelledButton*)data;
		btn.text = "Achievements";
		btn.col = { 128, 128, 128, 255 };
		}, &mAchievement);
	// Set "Exit" button interactions
	mExit.SetOnIn([](void* data) {
		LabelledButton& btn = *(LabelledButton*)data;
		btn.col = { 196, 128, 128, 255 };  // Dark red
		}, &mExit);

	mExit.SetOnOut([](void* data) {
		LabelledButton& btn = *(LabelledButton*)data;
		btn.col = { 255, 255, 255, 255 };  // White
		}, &mExit);
...
void TitleScene::OnExit()
{
	...
	Widget::Unregister(&mAchievement);
...
void TitleScene::OnUpdate(float dt)
{
	...
	if (mAchievement.IsMouseOver())
	{
		...
		float b = cosf(tt + PI * 0.67f) * 0.5f + 0.5f;
		Color color{ r * 255, g * 255, b * 255, 255 };
		mAchievement.colOver = color;
	}
	if (mExit.IsMouseOver())
	{
		const float tt = TotalTime();
		float r = cosf(tt + PI * 1.00f * 0.5f) + 0.5f;
		float g = cosf(tt + PI * 0.33f) * 0.5f + 0.5f;
...
void GameScene::OnUpdate(float dt)
{
	...
	if (IsKeyDown(Bindings::Get(LEFT)))
	{
	...
	if (IsKeyDown(Bindings::Get(RIGHT)))
	{
		mShip.velocity = mShip.velocity + mShip.direction * mShip.acceleration * dt;
	...
	if (IsKeyDown(Bindings::Get(TELEPORT)))
	...
	bool isFiring = (IsKeyPressed(Bindings::Get(SHOOT)));
